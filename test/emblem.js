...
describe('ERC721', () => {
  ...
  describe('Mint', ()=>{
    ...
    it('should verify signature and hash in handler', async () => {
      ...
      expect(address).to.equal(util.witness)
    })
    ...
    it('should fail to mint with signature if signer is not a witness', async () => {
      ...
      var provider = util.selectProvider("mainnet")
      var web3 = new Web3(provider)
      let hash = web3.utils.soliditySha3(emblemAddress, util.erc20.address, 0, util.deployer.address, 123, 111, 1)
      let sig = await sign(web3, hash)
      let tx = util.handler.buyWithSignedPrice(emblemAddress, util.erc20.address, 0, util.deployer.address, 123, 111, sig, util.serializeUintToBytes(0), 1)
    ...
    it('should mint via handler with signature if signer is a witness', async () => {
      ...
      await util.handler.buyWithSignedPrice(emblemAddress, util.erc20.address, 0, util.deployer.address, 123, 111, sig, util.serializeUintToBytes(0), 1)
    ...
    it('should mint via handler with signed price', async () => {
      ...
      var provider = util.selectProvider("mainnet")
      var web3 = new Web3(provider)
      let hash = web3.utils.soliditySha3(emblemAddress, covalAddress, 0, util.deployer.address, 123, 111, 1)
      let sig = await sign(web3, hash)
      let balance = await emblemContract.balanceOf(util.deployer.address)
      expect(balance.toNumber()).to.equal(0)
      await emblemContract.transferOwnership(util.handler.address)
      await util.erc20.transferOwnership(util.handler.address)
      await util.handler.buyWithSignedPrice(emblemAddress, covalAddress, 0, util.deployer.address, 123, 111, sig, util.serializeUintToBytes(0), 1)
      balance = await emblemContract.balanceOf(util.deployer.address)
      expect(balance.toNumber()).to.equal(1)
    })
  })
