...
describe('ERC1155', () => {
    ...
    //     console.log("owner", await ERC1155.owner())
    ...
    //         let calculatedSerial0 = await ethers.utils.keccak256(AbiCoder.encode([ "uint","address","uint"], [789, util.deployer.address, 0]))
    //         let calculatedSerial1 = await ethers.utils.keccak256(AbiCoder.encode([ "uint","address","uint"], [789, util.deployer.address, 1]))
    ...
    //         let calculatedSerial2 = await ethers.utils.keccak256(AbiCoder.encode([ "uint","address","uint"], [789, util.deployer.address, 2]))
    ...
    //         let deployerSerialCount = await ERC1155.ownerSerialCount(util.deployer.address)
    //         let bobSerialCount = await ERC1155.ownerSerialCount(util.bob.address)
    //         expect(deployerSerialCount).to.equal(2)
    //         expect(bobSerialCount).to.equal(0)
    ...
    //         deployerSerialCount = await ERC1155.ownerSerialCount(util.deployer.address)
    //         expect(deployerSerialCount).to.equal(1)
    //         bobSerialCount = await ERC1155.ownerSerialCount(util.bob.address)
    //         expect(bobSerialCount).to.equal(1)
    ...
    //         expect(ownerOfSerial0).to.equal(util.bob.address)
    //         deployerSerialCount = await ERC1155.ownerSerialCount(util.deployer.address)
    //         expect(deployerSerialCount).to.equal(0)
    //         bobSerialCount = await ERC1155.ownerSerialCount(util.bob.address)
    //         expect(bobSerialCount).to.equal(2)
    //     })
    ...
    describe('V2 Upgradable', ()=>{
        it('should mint', async ()=>{
            ...
            await util.erc1155Clone.toggleOverloadSerial()
            await util.erc1155Clone.mint(util.deployer.address, 789, 1)
            let currentBalance = await util.erc1155Clone.balanceOf(util.deployer.address, 789)
            expect(currentBalance).to.equal(1)
        })
        it('can mint with provided serial', async ()=>{
            await util.deployERC1155Upgradable(util.handler)
            await util.erc1155Clone.mintWithSerial(util.deployer.address, 789, 1, util.serializeUintToBytes(123))
            let currentBalance = await util.erc1155Clone.balanceOf(util.deployer.address, 789)
            expect(currentBalance).to.equal(1)
            let currentSerial = await util.erc1155Clone.getFirstSerialByOwner(util.deployer.address, 789)
            expect(currentSerial).to.equal(123)
        })
        it('should not mint with duplicate serial', async ()=>{
            await util.deployERC1155Upgradable(util.handler)
            await util.erc1155Clone.mintWithSerial(util.deployer.address, 789, 1, util.serializeUintToBytes(123))
            let tx = util.erc1155Clone.mintWithSerial(util.deployer.address, 789, 1, util.serializeUintToBytes(123))
            await expect(tx).to.be.revertedWith("Serial number already used")
        })
        it('should not standard mint when overridable', async ()=>{
            await util.deployERC1155Upgradable(util.handler)
            let tx = util.erc1155Clone.mint(util.deployer.address, 789, 1)
        ...
        it('should not mint amounts over 1 with duplicate encoded serials', async ()=>{
            await util.deployERC1155Upgradable(util.handler)
            let tx = util.erc1155Clone.mintWithSerial(util.deployer.address, 789, 2, util.serializeUintArrayToBytes([123, 123]))
            await expect(tx).to.be.revertedWith("Serial number already used")
        })
        it('should mint amounts over 1 with encoded serials', async ()=>{
            await util.deployERC1155Upgradable(util.handler)
            await util.erc1155Clone.mintWithSerial(util.deployer.address, 789, 2, util.serializeUintArrayToBytes([123, 456]))
            let serial1 = await util.erc1155Clone.getSerial(789, 0)
            let serial2 = await util.erc1155Clone.getSerial(789, 1)
            expect(serial1).to.equal(123)
            expect(serial2).to.equal(456)
        })
        it('should not mint batch with duplicate encoded serials', async ()=>{
            await util.deployERC1155Upgradable(util.handler)
            let tx = util.erc1155Clone.mintBatch(util.deployer.address, [789, 543], [1,1], util.serializeToByteArray([123, 123]))
            await expect(tx).to.be.revertedWith("Serial number already used")
        })
        it('should mint batch with encoded serials', async ()=>{
            await util.deployERC1155Upgradable(util.handler)
            await util.erc1155Clone.mintBatch(util.deployer.address, [789, 543], [1,1], util.serializeToByteArray([123, 456]))
            let serial1 = await util.erc1155Clone.getSerial(789, 0)
            let serial2 = await util.erc1155Clone.getSerial(543, 0)
            expect(serial1).to.equal(123)
            expect(serial2).to.equal(456)
        })
        it('should mint batch with encoded serials and amounts', async ()=>{
            await util.deployERC1155Upgradable(util.handler)
            await util.erc1155Clone.mintBatch(util.deployer.address, [789, 543], [1,2], util.serializeToByteArray([789, [123, 456]]))
            let serial1 = await util.erc1155Clone.getSerial(789, 0)
            let serial2 = await util.erc1155Clone.getSerial(543, 0)
            let serial3 = await util.erc1155Clone.getSerial(543, 1)
        ...
        it('should burn', async ()=>{
            ...
            await util.erc1155Clone.toggleOverloadSerial()
            await util.erc1155Clone.mint(util.deployer.address, 789, 1)
            await util.erc1155Clone.burn(util.deployer.address, 789, 1)
            let currentBalance = await util.erc1155Clone.balanceOf(util.deployer.address, 789)
        ...
        it('should claim', async ()=>{
            ...
            await util.erc1155Clone.mint(util.deployer.address, 789, 1)
            await util.erc1155Clone.setApprovalForAll(util.handler.address, true)
            await util.handler.registerContract(util.claimedUpgradable.address, 6)
            await util.handler.registerContract(util.erc1155Clone.address, 1)
            await util.erc1155Clone.registerContract(util.handler.address, 3)
            await util.claimedUpgradable.registerContract(util.handler.address, 3)
            await util.handler.claim(util.erc1155Clone.address, 789)
