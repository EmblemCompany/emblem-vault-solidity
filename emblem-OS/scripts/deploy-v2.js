...
async function main() {
  ...
  console.log("Deploying HustleERC20V2 with account:", deployer.address);
  ...
  console.log("\nDeploying HustleERC20V2 as transparent proxy...");

  // Deploy using transparent proxy pattern
  // First call initialize(), then initializeV2() will be called separately
  const proxy = await upgrades.deployProxy(
    HustleERC20V2,
    [deployer.address], // initialOwner for initialize()
    {
      kind: "transparent",
  ...
  console.log("Proxy deployed, now calling initializeV2()...");

  // Call initializeV2 to set up roles
  const tx = await proxy.initializeV2();
  await tx.wait();
  ...
  const hasMinterRole = await proxy.hasRole(MINTER_ROLE, owner);
  ...
  if (chainId) {
    ...
    try {
      ...
      console.log("You can verify manually with: npm run verify:mainnet");
    }
  }
